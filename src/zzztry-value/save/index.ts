...
function OValue<OS extends ObjectShape>(shape: OS) {
  const i = class ObjectValue implements ObjectRuntimeInstance<OS> {
    ...
    static deserialize<T extends ObjectConstructor<OS>>(
      ...
      for (const key in shape) {
        input[key] = (shape as any)[key].deserialize(serialized[key]);
      }
...
// const a: ObjectConstructor<any> = Transaction;

// const b: ObjectRuntimeProps<ObjectShape> = new Transaction({
//   in: new Money(2),
//   out: new Money(3),
// });
...
const rec2 = TransactionRecord.deserialize({
  in: { in: 3, out: 4 },
  out: { in: 3, out: 4 },
});

type ListShape = PrimitiveConstructor<any>[];
type ListSerialized<LS extends ListShape> = LS extends Array<infer U>
  ? U extends PrimitiveConstructor<infer U>
    ? PrimitiveSerialized<U>[]
    : never
  : never;

type ListRuntimeProps<LS extends ListShape> = {
  [K in keyof LS]: LS[K] extends PrimitiveConstructor<infer U>
    ? PrimitiveRuntime<U>
    : never;
};

type ListRuntimeInstance<LS extends ListShape> = {
  serialize(): ListSerialized<LS>;
};

type ListRuntime<LS extends ListShape> = ListRuntimeProps<LS> &
  ListRuntimeInstance<LS>;

type ListConstructor<LS extends ListShape> = {
  new (content: ListRuntimeProps<LS>): ListRuntime<LS>;
  deserialize(serialized: ListSerialized<LS>): ListRuntime<LS>;
};

function LValue<LS extends ListShape>(shape: LS) {
  return class ListValue extends Array {
    constructor(content: ListRuntimeProps<LS>) {
      super();
    }
    ...
    static deserialize<T extends ListConstructor<LS>>(
      this: T,
      serialized: ListSerialized<LS>
    ) {
...
class Extract extends LValue([Money]) {}

new Extract([new Money(2)]);

const e = Extract.deserialize([2, 3]);
